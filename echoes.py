...
def execute_db_query(query, params=(), retry_count=3):
...
def ingest_chat_document(content, character_name):
    ...
    cursor = execute_db_query("SELECT rag_doc_ids FROM characters WHERE name = ?", (character_name,))
    result = cursor[0] if cursor else None
    if result:
        current_ids = json.loads(result[0])
        current_ids.extend(doc_ids)
        execute_db_query("UPDATE characters SET rag_doc_ids = ? WHERE name = ?", (json.dumps(current_ids), character_name))
...
def retrieve_rag_docs(character_name):
    ...
    result = execute_db_query(query, (character_name,))
    if not result:
        return []
    ...
    results = execute_db_query(query, tuple(rag_doc_ids))
...
async def chat(request: ChatRequest):
    ...
    result = execute_db_query(query, (request.character_name,))
    if not result:
...
async def get_chat_history(character_name: str):
    ...
    results = execute_db_query(query, (character_name,))
...
async def create_character(
    ...
    try:
        ...
        # --- FIX: Update in-memory caches and FAISS index immediately ---
...
async def delete_character(character_name: str):
    with get_db_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT rag_doc_ids FROM characters WHERE name = ?", (character_name,))
        result = cursor.fetchone()
        
        if not result:
            raise HTTPException(status_code=404, detail="Character not found")
        
        rag_doc_ids = json.loads(result[0])
        
        if rag_doc_ids:
            placeholders = ",".join("?" for _ in rag_doc_ids)
            cursor.execute(f"DELETE FROM rag_docs WHERE id IN ({placeholders})", tuple(rag_doc_ids))
        
        cursor.execute("DELETE FROM characters WHERE name = ?", (character_name,))
        conn.commit()
    
    # Delete the associated FAISS file
