...
class Media {
  ...
  update(scroll, direction) {
    ...
    // Enhanced center scaling with smooth curve
    const normalizedDistance = Math.min(Math.abs(x) / H, 1);
    const scaleFactor = 1 + 0.3 * Math.pow(1 - normalizedDistance, 2); // Quadratic falloff
    
    this.plane.scale.set(
      this.baseScaleX * scaleFactor,
      this.baseScaleY * scaleFactor,
      1
    );
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Store base scale for center scaling
